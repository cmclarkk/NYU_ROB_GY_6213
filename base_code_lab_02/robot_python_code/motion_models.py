...
class MyMotionModel:
    ...
    def step_update(self, encoder_counts, steering_angle_command, delta_t):
        ...
        distance += random.normalvariate(
            0,
            math.sqrt(
                variance_distance_travelled_s(encoder_counts - self.last_encoder_count)
            ),
        )  # convert to distance travelled since last step, and add noise based on variance of distance travelled
        ...
        )  # convert to equivalent speed command for the model, and add noise based on variance of rotational velocity
